...
| `server_version` | `5.5.30` | `8.0.36` | 需與後端 MySQL 版本對齊 |
...
**MySQL 8.0（必須指定 `mysql_native_password`）：**

```sql
-- MySQL 8.0 預設認證插件為 caching_sha2_password，ProxySQL 2.5.x 不支援
-- 建立帳號時必須指定 mysql_native_password，否則監控與連線會失敗
-- 注意：mysql_native_password 在 MySQL 8.4 已完全移除，僅適用於 MySQL 8.0.x
...
- **MySQL 8.0 認證插件**：MySQL 8.0 預設使用 `caching_sha2_password`，ProxySQL 2.5.x 不支援此插件。建立 `monitor`、`proxysql`、`replication` 等帳號時必須指定 `IDENTIFIED WITH mysql_native_password`，否則 ProxySQL 監控與連線會出現 `Authentication plugin 'caching_sha2_password' cannot be loaded` 錯誤。**若使用 MySQL 8.4，請改用 `feat/mysql84-support` 分支**（mysql_native_password 在 8.4 已完全移除）
